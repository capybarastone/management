...
<div class="flex min-h-screen items-center justify-center bg-background">
	<div class="w-full max-w-md rounded-lg border border-border bg-card p-8 shadow-sm">
		<div class="mb-6 flex items-center gap-3">
			<img src="/favicon.svg" alt="Herd" class="h-8 w-8" />
			<h1 class="text-2xl font-bold text-foreground">Initial Setup</h1>
		</div>

		<p class="mb-6 text-sm text-muted-foreground">
			Create your first admin account to secure the management interface.
		</p>
		...
		<form method="POST" class="space-y-4">
			...
			{#if form?.success}
				<div class="rounded-md bg-green-500/10 p-3 text-sm text-green-600">
					User created! <a href="/login" class="underline">Sign in</a>
				</div>
			{/if}
			...
			<div>
				...
				<input
					...
					required
					class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
				/>
			</div>
